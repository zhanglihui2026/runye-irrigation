...
| `peanut-tool.html` | 润野 · 七彩花生水肥与植保决策（地块/生长参数、追肥 EC、底肥配置、天气联动、生育期对照、当前植保节点与全期追肥计划） |
| `数字农业.html` | 数字农业 · 七彩花生水肥与植保决策（桌面端嵌入 peanut-tool.html，移动端直达工具页） |
| `workbench.html` | 数字农业工作台（地块全周期管理，引用统一植保节点数据） |
| `qicai-huasheng/schedule-core.js` | 七彩花生植保节点统一数据源，供数字农业工具、工作台和植保详情弹窗引用 |
